        ...
        if len(apt_packages):
            ...
            #        http2 = ("http2" if set(["nginx-mainline"]).issubset(set(apt_packages)) else "spdy")
                    ...
                    if (EEVariables.ee_platform_codename == 'jessie' or EEVariables.ee_platform_codename == 'trusty' or EEVariables.ee_platform_codename == 'xenial' or EEVariables.ee_platform_codename == 'bionic') and (not
                        os.path.isfile("/etc/nginx/common/php74.conf")):
                        #data = dict()
                        Log.debug(self, 'Writting the nginx configuration to '
                              'file /etc/nginx/common/locations-php74.conf')
                        ee_nginx = open('/etc/nginx/common/locations-php74.conf',
                                    encoding='utf-8', mode='w')
                        self.app.render((data), 'locations-php74.mustache',
                                    out=ee_nginx)
                        ee_nginx.close()

                        Log.debug(self, 'Writting the nginx configuration to '
                        'file /etc/nginx/common/php74.conf')
                        ...
                        Log.debug(self, 'Writting the nginx configuration to '
                              'file /etc/nginx/common/w3tc-php74.conf')
                        ee_nginx = open('/etc/nginx/common/w3tc-php74.conf',
                                encoding='utf-8', mode='w')
                        self.app.render((data), 'w3tc-php74.mustache', out=ee_nginx)
                        ee_nginx.close()

                        Log.debug(self, 'Writting the nginx configuration to '
                                'file /etc/nginx/common/wpcommon-php74.conf')
                        ee_nginx = open('/etc/nginx/common/wpcommon-php74.conf',
                                    encoding='utf-8', mode='w')
                        self.app.render((data), 'wpcommon-php74.mustache',
                                    out=ee_nginx)
                        ee_nginx.close()

                        Log.debug(self, 'Writting the nginx configuration to '
                              'file /etc/nginx/common/wpfc-php74.conf')
                        ee_nginx = open('/etc/nginx/common/wpfc-php74.conf',
                                encoding='utf-8', mode='w')
                        self.app.render((data), 'wpfc-php74.mustache',
                            out=ee_nginx)
                        ee_nginx.close()

                        Log.debug(self, 'Writting the nginx configuration to '
                              'file /etc/nginx/common/wpsc-php74.conf')
                        ee_nginx = open('/etc/nginx/common/wpsc-php74.conf',
                                encoding='utf-8', mode='w')
                        self.app.render((data), 'wpsc-php74.mustache',
                                out=ee_nginx)
                        ee_nginx.close()

                        Log.debug(self, 'Writting the nginx configuration to '
                                  'file /etc/nginx/common/redis-php74.conf')
                        ee_nginx = open('/etc/nginx/common/redis-php74.conf',
                                     encoding='utf-8', mode='w')
                        self.app.render((data), 'redis-php74.mustache',
                                      out=ee_nginx)
                        ee_nginx.close()
                ...
                if EEAptGet.is_installed(self,'redis-server'):
                    ...
                    if (EEVariables.ee_platform_codename == 'trusty' or EEVariables.ee_platform_codename == 'xenial' or EEVariables.ee_platform_codename == 'bionic'):
                        if os.path.isfile("/etc/nginx/nginx.conf") and (not
                            os.path.isfile("/etc/nginx/common/redis-php8.conf")):
                            data = dict()
                            Log.debug(self, 'Writting the nginx configuration to '
                                  'file /etc/nginx/common/redis-php8.conf')
                            ee_nginx = open('/etc/nginx/common/redis-php8.conf',
                                        encoding='utf-8', mode='w')
                            self.app.render((data), 'redis-php8.mustache',
                                        out=ee_nginx)
                            ee_nginx.close()
            ...
            #setup nginx common folder for php8
            if self.app.pargs.php7:
                if os.path.isdir("/etc/nginx/common") and (not
                    os.path.isfile("/etc/nginx/common/php8.conf")):
                    data = dict()
                    Log.debug(self, 'Writting the nginx configuration to '
                              'file /etc/nginx/common/locations-php8.conf')
                    ee_nginx = open('/etc/nginx/common/locations-php8.conf',
                                    encoding='utf-8', mode='w')
                    self.app.render((data), 'locations-php8.mustache',
                                    out=ee_nginx)
                    ee_nginx.close()

                    Log.debug(self, 'Writting the nginx configuration to '
                      'file /etc/nginx/common/php8.conf')
                    ee_nginx = open('/etc/nginx/common/php8.conf',
                            encoding='utf-8', mode='w')
                    self.app.render((data), 'php8.mustache',
                            out=ee_nginx)
                    ee_nginx.close()

                    Log.debug(self, 'Writting the nginx configuration to '
                          'file /etc/nginx/common/w3tc-php8.conf')
                    ee_nginx = open('/etc/nginx/common/w3tc-php8.conf',
                            encoding='utf-8', mode='w')
                    self.app.render((data), 'w3tc-php8.mustache', out=ee_nginx)
                    ee_nginx.close()

                    Log.debug(self, 'Writting the nginx configuration to '
                                'file /etc/nginx/common/wpcommon-php8.conf')
                    ee_nginx = open('/etc/nginx/common/wpcommon-php8.conf',
                                    encoding='utf-8', mode='w')
                    ...
                    Log.debug(self, 'Writting the nginx configuration to '
                          'file /etc/nginx/common/wpfc-php8.conf')
                    ee_nginx = open('/etc/nginx/common/wpfc-php8.conf',
                                encoding='utf-8', mode='w')
                    self.app.render((data), 'wpfc-php8.mustache',
                            out=ee_nginx)
                    ee_nginx.close()

                    Log.debug(self, 'Writting the nginx configuration to '
                          'file /etc/nginx/common/wpsc-php8.conf')
                    ee_nginx = open('/etc/nginx/common/wpsc-php8.conf',
                                encoding='utf-8', mode='w')
                    self.app.render((data), 'wpsc-php8.mustache',
                                out=ee_nginx)
                    ee_nginx.close()

                if os.path.isdir("/etc/nginx/common") and (not os.path.isfile("/etc/nginx/common/redis-php8.conf")):
                    data = dict()
                    Log.debug(self, 'Writting the nginx configuration to '
                         'file /etc/nginx/common/redis-php8.conf')
                    ee_nginx = open('/etc/nginx/common/redis-php8.conf',
